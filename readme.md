# Proyecto cifrado de disco duro virtual seguro

- [x] Arquitectura cliente/servidor, realizándose ambos en Go a partir del código proporcionado.
- [x] Mecanismo de autentificación seguro (gestión de contraseñas, identidades y sesión).
- [x] Transporte de red seguro entre cliente y servidor (HTTPS).
- [x] Almacenamiento seguro (cifrado en descanso) en el servidor. //Se realizado por parte del cliente, deberiamos ver de que el servidor lo haga por su cuenta?
- [x] Gestión de los ficheros y carpetas (creación, borrado, etc.)

# Opcionales

- [ ] Cifrado punto a punto (el servidor no conoce las claves ni la información, todo el
      cifrado/descifrado se realiza en el cliente de forma local).
- [x] Gestión de metadatos de los ficheros (permisos, fechas, diferentes plataformas, etc.)
- [ ] Gestión de categorías, tags o filtrado de ficheros (si implica seguridad adicional).
- [ ] Posibilidad de compartir ficheros o carpetas con otros usuarios o grupos y hacerlos públicos.
- [ ] Sistema de notas o comentarios en los ficheros tanto del propio usuario como de otros.
- [ ] Gestión de diferentes roles o grupos de usuarios.
- [ ] Incorporación de clave pública en la autentificación de usuarios.
- [ ] Incorporación de firma digital para garantizar el origen de los datos.
- [ ] Sistema de comunicación privado (cifrado) entre usuarios.
- [x] Sistema de registro de eventos (logging), para mejorar la trazabilidad (remoto).
- [x] Sistema de copia de seguridad (backup), para recuperarse ante incidentes (remoto).
- [ ] Otros relacionadas con la seguridad y privacidad a consensuar con el profesorado.

# NOTA:

_Añadir en el .env:_

LOG_SERVER_ENDPOINT=http://localhost:3000/logs
LOG_SERVER_AUTH_ENDPOINT=http://localhost:3000/login
LOG_SERVER_USER=admin
LOG_SERVER_PASS=1234
LOG_SERVER_TOKEN=token-secreto
LOG_SERVER_MAX_ATTEMPTS=3
LOG_SERVER_PORT=3000
LOG_ENCRYPTION_KEY=0123456789abcdef0123456789abcdef

_Para ver desde terminal los logs no cifrados:_

```bash
curl.exe http://localhost:3000/logs/read `
-H "Authorization: Bearer token-secreto"
```
